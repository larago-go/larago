      ...
      <form class="mt-8 space-y-6" @submit.prevent="submit">
        <div class="rounded-md shadow-sm -space-y-px">
          <div>
            ...
            <input id="name" name="name" v-model="form.name" type="text" autocomplete="name" required="" class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-t-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" placeholder="Name" />
          </div>
          <div>
...
export default {
  ...
  await Connect.get("/auth/api/register")
    ...
    methods: {
      ...
      async submit() {
        ...
        Connect.defaults.headers.post['X-CSRF-Token'] = this.csrf;
         ...
         await Connect.post("/auth/signup", this.form)
